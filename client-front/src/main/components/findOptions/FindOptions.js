...
import AddOptionDetails from './addOptionDetails/AddOptionDetails';
...
const FindOptions = ({ setSelectedLink, link }) => {
  const {
  ...
  const handleSubmit = () => {
    const product = {
      optionTitle: detailsProducts.name,
      optionGoal: detailsProducts.goal,
      questionType: 'next',
      resultAI: '',
    };
    console.log(product);
    createOptions(product, currentUser, dispatch, 0);
    navigate('/option/explore');
  };
        ...
        >
          ...
          >
            <Typography variant="h6" component="span">
              {'Ask your question to find options '}
            </Typography>
          </Box>
          <Box sx={{ pb: 7, px: 1 }}>
            {
